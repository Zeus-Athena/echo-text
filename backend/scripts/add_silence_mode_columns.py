...
def migrate():
    ...
    try:
        ...
        if "silence_mode" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE user_configs ADD COLUMN silence_mode VARCHAR(20) DEFAULT 'manual'"
            )
        else:
        ...
        if "silence_prefer_source" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE user_configs ADD COLUMN silence_prefer_source VARCHAR(20) DEFAULT 'auto'"
            )
        else:
        ...
        if "silence_threshold_source" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE user_configs ADD COLUMN silence_threshold_source VARCHAR(20) DEFAULT 'default'"
            )
        else:
